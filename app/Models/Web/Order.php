...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        } else if($payment_method == 'wallet_pay'){

            $payment_method_name = 'wallet_pay';//$payments_setting->name;
            $wallet_pay = $order_price;
            $online_pay = 0;
            $payment_status='success';

        } else if($payment_method == 'wallet_pay_online'){

            $payment_method_name = 'wallet_pay_online';//$payments_setting->name;
            $wallet_pay = auth()->guard('customer')->user()->wallet_balance;//$order_price;
            $online_pay = $order_price - auth()->guard('customer')->user()->wallet_balance;
            $payment_status='success';

        } else if($payment_method == 'razorpay'){

            $wallet_pay = 0;
            $online_pay = $order_price;
        ...
        if ($payment_status == 'success') {            
            ...
            if($wallet_pay>0){
                $wallet_balance =  auth()->guard('customer')->user()->wallet_balance;
                if($online_pay>0){
                    $final_amt = $order_price-$wallet_balance;
                }else{
                    $final_amt = $wallet_balance-$order_price;
                }               
                $source_type = 'New Order';                
                $wallet_transaction = DB::table('user_wallet_transactions')->insertGetId([
                    'source_id'=>$orders_id,
                    'source_type'=>$source_type,
                    'deduction_type'=>'debit',
                    'amount'=>$order_price,
                    'amount_bf_transaction'=>$wallet_balance,
                    'amount_af_transaction'=>$final_amt,
                    'user_id'=>auth()->guard('customer')->user()->id,
                    'comment'=>'Amount has been debited for '.$source_type.' order'
                ]);               
                DB::table('users')->where('id',auth()->guard('customer')->user()->id)->update([
                    'wallet_balance'=>$final_amt
                ]);
            }
