...
//current time
	...
	public function sendBasketReminder(){
		$date = date('Y-m-d');
		$basketDetails = DB::table('customers_basket')->where('customers_basket.is_notification_sent','0')->where('customers_basket.customers_id','!=',0)->get();
		if(count($basketDetails)>0){
			foreach($basketDetails as $b_d){
				$cust_id = $b_d->customers_id;
				$cust_det = DB::table('users')->where('users.users_id',$cust_id)->get();
				if($cust_det[0]->email!="" && $cust_det[0]->email!=null){
					DB::table('customers_basket')->where('customers_basket.customers_id',$cust_id)->update(['customers_basket.is_notification_sent'=>'1']);
				}
			}
		}		
	}
	
	public function customerOrderAlert(){
		$date = date('Y-m-d');
		$orders = DB::table('orders')->orderby('orders.date_purchased','DESC')->groupby('orders.customers_id')->get();
		if(count($orders)>0){
			foreach($orders as $o_d){
				$date1=date_create($date);
				$date2=date_create($orders->date_purchased);
