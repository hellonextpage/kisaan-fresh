...
class OrdersController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (count($result['cart']) == 0) {
			...
			/*$u_id = auth()->guard('customer')->user()->id;
			$addresses = DB::table('user_to_address')->where('user_id',$u_id)->where('is_default',1)->get();
			

			if(!empty($addresses[0]->address_book_id)){

				$address_id = $addresses[0]->address_book_id;
				$address = $this->shipping->getShippingAddress($address_id);
				if(!empty($address)){
					$address = $address[0];

					$address->delivery_phone = auth()->guard('customer')->user()->mobile;
				}else{
					$address = '';
				}
			}*/
			...
			$result['address'] = array();//$this->shipping->getShippingAddress($address_id='');
    ...
    {
        $mobile = session('shipping_address')->delivery_phone;
        $payment_status = $this->order->place_order($request);
        if ($payment_status == 'success') {
            $msg = "Thank you for placing the order. ";
            $api_key = 'GcdobfzeyjHJvHVfus3Aw7ZRQM0XSU6mDIA2gpCt';
            $contacts = $mobile;//'8885344518';
            $from = 'LOCALM';
